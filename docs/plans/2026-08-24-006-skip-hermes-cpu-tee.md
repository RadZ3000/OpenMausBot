...
Status: **measured 2026-08-24 on Admin.** Not a product change. Path A gold
is still Hermes ACP ([005](2026-08-24-005-path-a-qwen3vl-8b-thinking-plan.md)).
...
| | Skip-Hermes 4B | Skip-Hermes 8B | Hermes ACP 8B |
|---|---|---|---|
| Tag | `qwen3-vl:4b` | `qwen3-vl:8b` | `qwen3-vl:8b` |
| Digest prefix | `1343d82ebee3` | `901cae732162` | `901cae732162` |
| When (CDT) | 15:54 | 16:09 | earlier 24 Aug |
| Wall | **166 s** (2 min 46 s) | **240 s** (4 min 0 s) | **~20.7 min** then cancel |
| Load | 7.0 s | 12.3 s | `session/new` ~6 s |
| Prefill | 30.0 s · 734 tok · ~24.5 tok/s | 69.8 s · 734 tok · ~10.5 tok/s | no ACP chunks |
| Decode | 129.3 s · 582 tok · ~4.5 tok/s | 158.1 s · 369 tok · ~2.3 tok/s | — |
| `done_reason` | `stop` | `stop` | `session/cancel` (`TURN_STALL_MS`) |
| Truncated | no | no | no (not a full-window miss) |
| Tools | `write_file` (`omb-tee.txt` / `8241`), `terminal` (`echo OMB-TEE-OK`), `vm_open` (`https://example.com`) | **same three, same args** | **none** |
| Thinking | 2046 chars | 1172 chars | unknown (no chunks) |
...
Hermes ACP 8B gold: thread `7105ac50-…`, `llama-server -c 32768`,
`computer: off`, default 20 min stall watchdog. **No** `tool_call`, not
`0xc0000409`. Same shape as Hermes 4B @ 16k on this box (cancel mid-prefill
of a ~6k agent prompt). UI “hey” through Hermes still sits at 0 tok —
that path still sends the Hermes catalog.

## What this means

The 20 min Retry card is **Hermes’s agent prompt on CPU**, not 4B/8B
weights being unable to emit tools. Skip-Hermes compact catalog finishes
on this box. Chat in the app still goes through Hermes until someone
asks for a first-party Ollama loop.
