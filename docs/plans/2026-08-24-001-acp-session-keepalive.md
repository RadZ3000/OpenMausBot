...
- **Fingerprint** (not secrets): `cwd`, **picker** model (`turn.model`, not `resolveTurnModel` — the harness skip-last-look check cannot see that rewrite), MCP **names** (computer/agents/composio/image/phone), and **approvalMode** (Cursor bakes Ask/Auto/Full into argv; a parked child must not keep `--force` / `--auto-review`). If the user turns Local VM on mid-thread, miss and respawn so `session/new` can mount MCP.
