...
type InternalCapability = {
  ...
  generation: string;
  depth: number;
  kind: "agents" | "connectors" | "computer" | "browser" | "imageGen";
...
function imageGenConfigured(): boolean {
  return Boolean(cfg.imageGen?.key || cfg.imageGen?.apiKey || cfg.imageGen?.baseUrl);
}

function imageGenIntegration(botId: string, threadId: string, directory: string, generation: string) {
  const token = mintInternalCapability({
    botId,
    threadId,
    generation,
  ...
  const imageKey = cfg.imageGen?.key || cfg.imageGen?.apiKey;
  if (imageKey) env.OMB_IMAGE_API_KEY = imageKey;
...
function wrapLocalComputerForModel<T extends { args: string[]; env?: Record<string, string> }>(
  launch: T,
  model: string,
  computerKind: "box" | "vps" | "vm" | "local" | null,
): T {
  if (decodeInjectId(model)) {
    const wrapped = wrapComputerMcpForLocalModel(launch);
  ...
  if (computerKind === "vm" || computerKind === "vps") {
    return wrapComputerMcpForFrontier(launch);
  }
  return launch;
}
...
async function startTurn(
  ...
  void (async () => {
    try {
      ...
      if (bot.composio !== false && composio.configured(cfg) && instance.adapter.capabilities.composioMcp === true && !localInjectOmitsConnectedApps(model)) {
      ...
      if (
        ...
        integrations.imageGen = imageGenIntegration(bot.id, threadId, imageDirectory, dispatchClaimId);
      }
      ...
      if (integrations.localComputer) {
        integrations.localComputer = wrapLocalComputerForModel(integrations.localComputer, model, computerKind);
      }
      ...
      const prompt = buildSystemPrompt(persona, liveBot?.soul ?? bot.soul ?? "", [
        ...
        { id: "imageGen", label: "Images", text: imageGenPrompt(integrations.imageGen) },
      ...
      const reuseAcp =
        ...
        instance.adapter.hasIdleSession?.(
          sessionKey,
          acpFingerprintFromTurn({ threadId, text: dispatchContext.turnText, model, integrations, cwd }),
        ) === true;
      ...
      const dispatch = await guardTurnDispatch(instance.adapter.sendTurn({
        ...
        sessionKey,
        botId: bot.id,
        text: withComputerObservation(dispatchContext.turnText, reuseAcp ? "" : lastLook),
...
async function runGroupMemberTurn(
  ...
  try {
    if (bot.composio !== false && composio.configured(cfg) && instance.adapter.capabilities.composioMcp === true && !localInjectOmitsConnectedApps(bot.modelSelection.model)) {
  ...
  if (readyBot.computer === "vm") {
    ...
    integrations.localComputer = wrapLocalComputerForModel(
      containerComputerMcp(
        vm.runtime,
  ...
  const outsideRoom = integrations.agents
    ...
    : [];
  const system = [
    `You are ${bot.name}, a bot in the room "${readyGroup.name}" in ${PRODUCT_NAME}.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
  ...
  if (
    ...
    integrations.imageGen = imageGenIntegration(bot.id, threadId, cwd ?? workspace!, internalGeneration);
  }
  {
    const drift = checkSoulDrift(bot.id, bot.soul ?? "", bot.soulHash ?? "");
    if (drift.drift !== Boolean(bot.soulDrift)) store.patchBot(bot.id, { soulDrift: drift.drift });
...
function configStatus() {
  return {
    ...
    // Ours: the workspace generate_image proxy. Its credential is deliberately
    // separate from the avatar providers above, so it reports separately too.
    imageGenProxy: { configured: imageGenConfigured() },
  ...
  try {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      const requiredCapabilityKind = path === "/api/internal/browser/mcp"
        ...
        ? "connectors"
        : path === "/api/internal/computer-control" || path === "/api/internal/computer-look"
          ? "computer"
      ...
      if (method === "POST" && path === "/api/internal/images") {
        ...
        if (!internalCapabilityIsActive(internalCapability)) {
          return json(res, 401, { error: "the internal turn capability has expired" });
        }
        if (body && typeof body === "object" && !Array.isArray(body)) {
          for (const key of ["fromBotId", "botId"] as const) {
            if (body[key] !== undefined && String(body[key]) !== internalSender.id) {
      ...
      if (path === "/api/internal/computer-look") {
        ...
        if (threadId !== internalCapability.threadId) {
          return json(res, 403, { error: "the internal capability belongs to a different conversation" });
        }
        if (method === "GET") {
        ...
        if (method === "PUT") {
          const parsed = computerLookWriteSchema.safeParse(await readInternalBody());
    ...
    // POST /api/engines/api-key — Path B: enable the saved-key engine.
