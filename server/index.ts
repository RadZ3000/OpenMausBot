...
async function defaultSelection() {
  ...
  const pick = available.find((d) => d.driverKind === PREFERRED_ENGINE) ?? available.find((d) => d.driverKind === "claudeAgent") ?? available[0];
...
async function startTurn(
  ...
  void (async () => {
    try {
      ...
      } else if (integrations.localComputer && (computerKind === "vm" || computerKind === "vps")) {
...
async function runGroupMemberTurn(
  ...
  if (
    ...
    integrations.imageGen = imageGenIntegration(bot.id, threadId, cwd ?? workspace!);
  }
  const roomSystem =
    system +
    (integrations.browser ? BUILT_IN_BROWSER_SYSTEM_PROMPT : "") +
    ...
    if ((method === "PUT" || method === "PATCH") && path === "/api/config") {
      ...
      try {
        if (externalSecretStorage) {
          ...
          if (persisted.imageGen?.apiKey !== undefined) persisted.imageGen.apiKey = "";
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`${PRODUCT_NAME} server on http://127.0.0.1:${PORT}`);
