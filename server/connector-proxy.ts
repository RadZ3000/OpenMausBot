...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (requests.length) {
      ...
      send(textResult(
        id,
        `${PRODUCT_NAME} showed the user a secure connection card for ${labels}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
