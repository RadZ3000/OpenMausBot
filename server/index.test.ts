...
describe("harness HTTP API", () => {
  ...
  it("rejects non-loopback authorities while accepting IPv4 and IPv6 loopback forms", async () => {
    ...
    expect(Reflect.get(Object(Reflect.get(Object(brand.body), "brand")), "name")).toBe(PRODUCT_NAME);
  ...
  it("injects the bot's standing instructions (soul) into a real turn, directly after the persona", async () => {
    ...
    try {
      ...
      expect(system.startsWith(`You are Kiwi, a personal bot in ${PRODUCT_NAME}. Role: Tracker.`)).toBe(true);
      const persona = `You are Kiwi, a personal bot in ${PRODUCT_NAME}. Role: Tracker.`;
  ...
  it("does not coach an ordinary request even when the bot profile is blank", async () => {
    ...
    try {
      ...
      expect(system.startsWith(`You are Blank, a personal bot in ${PRODUCT_NAME}.`)).toBe(true);
  ...
  it("never removes an unmanaged container that squats on a bot's exact Local VM name", async () => {
    ...
    try {
      ...
      expect(removed.body.error).toMatch(new RegExp(`not created by ${PRODUCT_NAME}.*remove it manually`, "i"));
...
describe("bot memory API", () => {
  ...
  it("previews the system prompt the model will see, section by section", async () => {
    ...
    try {
      ...
      expect(before.status).toBe(200);
      const persona = `You are Kiwi, a personal bot in ${PRODUCT_NAME}. Role: Tracker. About: Files bugs.`;
