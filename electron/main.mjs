...
app.whenReady().then(async () => {
  ...
  if (app.isPackaged) {
    try {
      ...
      dialog.showErrorBox(
        ...
        error?.message ?? `Another process is using this ${productName()} data folder.`,
      );
      app.quit();
      return;
    }
  ...
  if (app.isPackaged) {
    app.setAsDefaultProtocolClient(process.env.OMB_PROTOCOL_SCHEME?.trim() || "openmausbot");
  ...
  if (!desktopRemoteAccess && app.isPackaged && (composioBrokerUrl() || inferenceBrokerUrl()) && !credentialStoreUnavailable) {
