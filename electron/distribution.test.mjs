...
describe("distributionEnv", () => {
  ...
  it("carries a baked preference through to the server", () => {
    expect(distributionEnv({ defaultEngine: "hermesAgent", defaultModel: "ollama::qwen3:4b" }, {})).toEqual({
      OMB_DEFAULT_ENGINE: "hermesAgent",
      OMB_DEFAULT_MODEL: "ollama::qwen3:4b",
    });
  });
  ...
  it("carries an engine without a model, and a model without an engine", () => {
    ...
    expect(distributionEnv({}, { OMB_DEFAULT_MODEL: "ollama::qwen3:4b" })).toEqual({
      OMB_DEFAULT_MODEL: "ollama::qwen3:4b",
    });
  });
