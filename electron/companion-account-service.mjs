...
/** Packaged and unpackaged both fail closed until `controlPlaneUrl` is
 * plugged (extraMetadata → OMB_CONTROL_PLANE_URL) or the env is set.
 * Development must still opt into an exact HTTPS origin (or HTTP loopback
 * Worker) so a contributor never sends an OTP or bearer to an accidental
 * host. An explicitly invalid override disables the feature instead of
...
export function friendlyCompanionAccountError(error) {
  ...
  const message = code === "network_unavailable"
    ? `${productName()} could not reach its secure connection service. Check your internet and try again.`
    : (FRIENDLY_MESSAGES[code] ?? FRIENDLY_MESSAGES.request_failed);
