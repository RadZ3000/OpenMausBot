...
import { localInjectCannotAutoDriveComputer } from "@/lib/computer-routing";
    ...
    <div className="flex flex-col gap-4">
              ...
              onClick={() => {
                ...
                if (mode === "local" && derived.approvalMode === "auto") setLocalAutoWarning(bot.id);
                  ...
                  localInjectCannotAutoDriveComputer({
                    ...
                    computer: mode ?? undefined,
                    cloudBackend: bot.cloudBackend,
                    autoApprove: derived.approvalMode === "auto",
                  })
                ) {
                  patch({ computer: mode, autoApprove: false, approvalMode: "ask" });
                }
      ...
      <div className="flex items-center justify-between gap-4 rounded-xl bg-card p-4">
        <div>
          ...
          <div className="mt-0.5 text-[13px] text-ink-secondary">
            {!imageGenConfigured
              ? "Add an image key in App Settings before giving this bot the generate_image tool."
              : !canUseImageGen
                ? "This bot's current engine cannot generate images."
                : imageGenEnabled
                  ? "Let this bot make pictures with generate_image."
        ...
        <Switch
          ...
          disabled={!imageGenEnabled && (!imageGenConfigured || !canUseImageGen)}
          onClick={() => patch({ imageGen: !imageGenEnabled })}
          title={
            !imageGenEnabled && !imageGenConfigured
              ? "Add an image key in App Settings first"
              : !imageGenEnabled && !canUseImageGen
