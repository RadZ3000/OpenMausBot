    ...
    >
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-1.5 text-[13px]">
          {APP_REPOSITORY && <AboutLink href={APP_REPOSITORY} label="GitHub" />}
          {DOCS_URL && <AboutLink href={DOCS_URL} label="Docs" />}
          {RELEASES_URL && <AboutLink href={RELEASES_URL} label="Releases" />}
          {LICENSE_URL && <AboutLink href={LICENSE_URL} label="License" />}
        </div>
        <button
          ref={closeRef}
          type="button"
