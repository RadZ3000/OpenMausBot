...
export function LocalComputerSection() {
  ...
  return (
    <>
      ...
      <Card title="Setup" subtitle={`Once a container runtime is open, ${distribution.productName} prepares Cua and the VM for you.`}>
        <div className="flex flex-col gap-4">
          ...
          >
            {perBot ? (
              <div className="text-[13px] leading-relaxed text-ink-secondary">
                {perBotRuntimeUnsupported
                  ? `Apple container requires an explicit host port, so ${distribution.productName} will not guess or expose one. Install or start Docker or Podman for safe per-bot dynamic loopback ports.`
                  : <>
                      Choose <b className="text-ink">Local VM</b> for a bot, open that bot's Computer panel, then create its desktop there. {distribution.productName} assigns a private workspace and an available loopback viewer port automatically.
                    </>}
              </div>
            ) : needsRecreate ? (
              <>
      ...
      {unavailable && (
        <Card>
          <div className="flex gap-2 text-[13px] text-ink-secondary">
            ...
            <span>{distribution.productName} could not inspect the container runtime. Re-check, or review the app logs.</span>
          </div>
        </Card>
      )}
